...
export async function createClaim(playerId: string, key: string, amount: number) {
  ...
  // Enqueue AFTER the claim row has committed. pg-boss runs on its own connection,
  // so enqueuing inside the transaction races the chain worker: it can read the
  // not-yet-committed claim, see null, no-op, and strand the row in 'pending'
  // forever. If this enqueue is lost (queue down), the reconcile job re-enqueues it.
  await enqueueClaimTransfer({ claimId: String(result.claimId) });
  return result;
}
